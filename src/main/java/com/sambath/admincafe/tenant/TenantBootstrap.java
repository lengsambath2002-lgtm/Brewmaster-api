...
public class TenantBootstrap {
    ...
    @Bean
    ApplicationRunner tenantBootstrapRunner() {
    ...
    // Hibernate ddl-auto=update adds tenant_id columns as nullable. Existing rows
    // (legacy single-tenant data) need to be attributed to the default tenant
    // before @TenantId filtering hides them.
    ...
    // ddl-auto=update adds the new unique constraint but cannot drop the old
    // global one (uk_orders_date_daily_number). Without dropping it, two tenants
    // sharing the same (order_date, daily_number) would still collide.
